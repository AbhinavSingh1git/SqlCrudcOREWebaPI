...
namespace EmployeeWebApi
{
    public class Startup
    {
        ...
        public void ConfigureServices(IServiceCollection services)
        {
           ...
           services.AddDbContext<EmployeeContext>(
             opt => opt.UseSqlServer(Configuration.GetConnectionString("EmployeeConnection")), 
             ServiceLifetime.Singleton /*This is required for registering singelton*/
            );    
            //services.AddSingleton<IEmployeeRepo,EmployeeMockedRepository>();
            services.AddSingleton<IEmployeeRepo, EmployeeSqlRepository>();
            
              
            // services.AddControllers();
            services.AddControllers().AddNewtonsoftJson(s=>{
               s.SerializerSettings.ContractResolver=new CamelCasePropertyNamesContractResolver();
            });  /*Made changes for supporting Patch Update*/
